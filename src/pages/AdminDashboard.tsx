      ...
      <main className="flex-1 md:ml-64 flex flex-col min-w-0 w-full">
        ...
        <header className="h-16 border-b border-slate-200 dark:border-slate-800 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md flex items-center justify-between px-4 md:px-8 sticky top-0 z-10">
          <div className="flex items-center gap-3 md:hidden">
            ...
            <h2 className="text-base font-bold">어드민 대시보드</h2>
          </div>
          <h2 className="text-lg font-bold hidden md:block">어드민 대시보드</h2>
          <div className="flex items-center gap-4">
            <div className="relative">
              ...
              <input className="pl-10 pr-4 py-2 bg-slate-100 dark:bg-slate-800 border-none rounded-xl text-sm focus:ring-2 focus:ring-primary w-40 md:w-64 text-slate-900" placeholder="수업 검색..." type="text" />
            </div>
            <button onClick={handleLogout} className="md:hidden text-slate-400 hover:text-primary transition-colors flex items-center">
              <span className="material-symbols-outlined text-xl">logout</span>
            </button>
          </div>
        </header>

        {/* 모바일 하단 네비게이션 대체 또는 상단 네비게이션 (필요시) */}
        <div className="md:hidden flex overflow-x-auto border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 sticky top-16 z-10 custom-scrollbar">
          <a className="flex-shrink-0 flex items-center gap-2 px-4 py-3 text-sm font-medium border-b-2 border-primary text-primary" href="#">
            <span className="material-symbols-outlined text-lg">dashboard</span>
            <span>대시보드</span>
          </a>
          <a className="flex-shrink-0 flex items-center gap-2 px-4 py-3 text-sm font-medium border-b-2 border-transparent text-slate-600 dark:text-slate-400" href="#">
